...
| `--custom-rm-path` | str | None | 自定义 Reward 函数路径。同步函数走 `RewardWorker` 进程池，异步函数直接 `await`。单样本签名：`def custom_rm(args, sample) -> float \| dict`；`--group-rm` 时签名：`def custom_rm(args, samples) -> list[float \| dict]` |
