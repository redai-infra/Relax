...
def test_runner_accepts_caller_selected_absolute_artifact_directories() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    assert "/root/task22-local/runs" not in source
    assert "/root/autodl-fs/task22" not in source
