...
def _p3o_kl_mode_choices() -> list[str]:
    """Read the CLI choices without importing Relax's Megatron dependency
    chain."""
    tree = ast.parse(ARGUMENTS_PATH.read_text(encoding="utf-8"))
    call = next(
        node
        ...
        and node.args[0].value == "--p3o-kl-mode"
    )
...
def test_p3o_arguments_rejects_non_production_kl_modes():
    with pytest.raises(ValueError, match="proxy or proxy_safe"):
        validate_p3o_args(_p3o_args(p3o_kl_mode="exact"))


def test_p3o_arguments_expose_only_production_kl_modes():
    assert _p3o_kl_mode_choices() == ["proxy", "proxy_safe"]
