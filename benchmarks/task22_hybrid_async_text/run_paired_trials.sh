...
# Run three paired Task 22 trials. Each trial contains one baseline and one
# optimized component run with the same seed and workload.
...
run_component() {
    ...
    local monitor_pid=""
    local status=""
    local update_weight_buffer_size=""
    if [[ "${variant}" == baseline ]]; then
        update_weight_buffer_size="${BASELINE_BUFFER_SIZE}"
    else
        update_weight_buffer_size="${OPTIMIZED_BUFFER_SIZE}"
    fi
    mkdir -p "${run_dir}"
...

    {
        ...
        echo "reference_forward=enabled"
