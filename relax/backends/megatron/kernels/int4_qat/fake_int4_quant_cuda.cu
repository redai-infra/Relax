...
__device__ __host__ __forceinline__ int ceil_div(int a, int b) {
  return (a + b - 1) / b;
}
...
__global__ void int4_quant_1x32_kernel(
    const scalar_t *__restrict__ x, scalar_t *__restrict__ out,
    scalar_t *out_scale, scalar_t *out_zero, const int M, const int N,
    const int stride_xm, const int stride_xn, const int stride_om,
    const int stride_on, const int stride_osm, const int stride_osn,
    const int stride_ozm, const int stride_ozn, bool sym) {
  ...
  for (int item = warp_id; item < needed_warps; item += WARPS_PER_BLOCK) {
    ...
    if (col < N) {
      ...
      out_scale[row * stride_osm + item * stride_osn] =
          static_cast<scalar_t>(scale);
      if (!sym) {
        out_zero[row * stride_ozm + item * stride_ozn] =
            static_cast<scalar_t>(zero);
      }
    }
  }
}
...
template <typename scalar_t>
__global__ void int4_quant_32x1_kernel(
    const scalar_t *__restrict__ x, scalar_t *__restrict__ out,
    scalar_t *out_scale, scalar_t *out_zero, const int M, const int N,
    const int stride_xm, const int stride_xn, const int stride_om,
    const int stride_on, const int stride_osm, const int stride_osn,
  ...
  for (int item = warp_id; item < N; item += WARPS_PER_BLOCK) {
    ...
    if (row < end_row) {
      out[row * stride_om + col * stride_on] = static_cast<scalar_t>(val);
      out_scale[blockIdx.x * stride_osm + item * stride_osn] =
          static_cast<scalar_t>(scale);
      if (!sym) {
        out_zero[blockIdx.x * stride_ozm + item * stride_ozn] =
            static_cast<scalar_t>(zero);
      }
    }
  }
...
__global__ void int4_quant_common_kernel(
    ...
    const int stride_xm, const int stride_xn, const int stride_om,
    const int stride_on, const int stride_osm, const int stride_osn,
    const int stride_ozm, const int stride_ozn, const int BLOCK_M,
    const int BLOCK_N, bool sym) {
...
// dispatch
template <typename scalar_t>
void launch_int4_quant_kernel(const scalar_t *x, scalar_t *out,
                              scalar_t *out_scale, scalar_t *out_zero, int M,
                              int N, const int stride_xm, const int stride_xn,
                              ...
                              int block_m, int block_n, bool sym,
                              cudaStream_t stream) {
  constexpr int WARPS_PER_BLOCK = 8;
  ...
  if (block_m == 1 && block_n == 32) {
    ...
    int4_quant_1x32_kernel<scalar_t><<<grid, block, 0, stream>>>(
        x, out, out_scale, out_zero, M, N, stride_xm, stride_xn, stride_om,
        stride_on, stride_osm, stride_osn, stride_ozm, stride_ozn, sym);
    ...
    int4_quant_common_kernel<scalar_t><<<grid, block, 0, stream>>>(
        x, out, out_scale, out_zero, M, N, stride_xm, stride_xn, stride_om,
        stride_on, stride_osm, stride_osn, stride_ozm, stride_ozn, block_m,
        block_n, sym);
  }
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
fake_int4_quant_cuda(torch::Tensor &x, std::vector<int64_t> &block_size,
                     bool sym) {
  ...
  TORCH_CHECK(block_m > 0 && block_n > 0,
              "Block sizes must be positive, got block_m=", block_m,
              ", block_n=", block_n);
  TORCH_CHECK((block_m * block_n) % 32 == 0, "block_m * block_n (",
              block_m * block_n,
              ") must be divisible by 32. "
              "But got a ",
              block_m, "x", block_n, " block.");

  auto out = torch::empty_like(x);
  auto out_scale =
      torch::empty({ceil_div(M, block_m), ceil_div(N, block_n)}, x.options());
  auto out_zero = torch::empty_like(out_scale);

  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();

  AT_DISPATCH_FLOATING_TYPES_AND(
      at::ScalarType::BFloat16, x.scalar_type(), "int4_quant_cuda", [&] {
        launch_int4_quant_kernel<scalar_t>(
            x.const_data_ptr<scalar_t>(), out.data_ptr<scalar_t>(),
            out_scale.data_ptr<scalar_t>(), out_zero.data_ptr<scalar_t>(), M, N,
            x.stride(0), x.stride(1), out.stride(0), out.stride(1),
            out_scale.stride(0), out_scale.stride(1), out_zero.stride(0),
            out_zero.stride(1), block_m, block_n, sym, stream);
      });
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("fake_int4_quant_cuda", &fake_int4_quant_cuda,
        "fake INT4 quantization cuda");
}
