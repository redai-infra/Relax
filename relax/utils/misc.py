...
def create_namespace(args_dict: Dict[str, Any]) -> Namespace:
    ...
    Returns:
        Namespace: An argparse.Namespace object with the given arguments
    """
...
def should_run_periodic_action(
    ...
    Args:
        ...
        num_rollout: Optional total rollout count. When provided with an
            enabled interval, the final rollout is also a trigger even when it
            is not a periodic or epoch boundary.
    """
    if interval is None:
        return False
...
def group_by(iterable, key=None):
    """Similar to itertools.groupby, but do not require iterable to be
    sorted."""
...
@lru_cache()
def get_hf_config(hf_checkpoint: str):
    from transformers import AutoConfig

    return AutoConfig.from_pretrained(hf_checkpoint)
