...
def count_unknown_letters_in_expr(expr: str):
    ...
    letters_in_expr = set([x for x in expr if x.isalpha()])  # noqa: C403
...
def split_tuple(expr: str):
    """Split the elements in a tuple/interval, while handling well-formatted
    ...
    if (
        ...
        and all([ch not in expr[1:-1] for ch in TUPLE_CHARS])  # noqa: C419
    ):
