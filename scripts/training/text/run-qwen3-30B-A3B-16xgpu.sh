...
CKPT_ARGS=(
   ...
   --load ${EXP_DIR}/Qwen3-30B-A3B_mcore_16xgpu
   --save ${EXP_DIR}/Qwen3-30B-A3B_mcore_16xgpu
   --save-interval 50
...
WANDB_ARGS=(
   ...
   --tb-experiment-name qwen3-30B-A3B-16x-sync-${now}
)
...
ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://${HOST_IP}:8265" \
   ...
   "${EVAL_ARGS[@]}" \
   "${SGLANG_ARGS[@]}" \
   "${MISC_ARGS[@]}"  2>&1 | tee log/qwen3-30B-A3B-GRPO-gpu16-sync-${now}.log
