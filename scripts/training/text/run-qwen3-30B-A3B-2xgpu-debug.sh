...
#
# Real-model 2xGPU training smoke for Qwen3-30B-A3B on ROCm/CUDA hosts.
...
export RAY_TMPDIR="${RAY_TMPDIR:=/tmp/ray-relax-real-smoke-$$}"
...
ASSET_DIR="${RELAX_SMOKE_ASSET_DIR:=/tmp/relax-real-qwen3-smoke}"
WORK_DIR="${RELAX_SMOKE_WORK_DIR:=${ASSET_DIR}/run-${DATE}}"
REAL_HF_MODEL_DIR="${REAL_HF_MODEL_DIR:=/mnt/dcgpuval/models/Qwen/Qwen3-30B-A3B-FP8}"
...
ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
   ...
   "${MODEL_ARGS[@]}" \
   "${TRAIN_ARGS[@]}" 2>&1 | tee "log/qwen3-30b-a3b-real-debug-gpu2-${DATE}.log"
