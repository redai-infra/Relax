...
#
# Usage:
#   bash scripts/training/text/run-qwen3-4B-8xgpu-hybrid-async.sh
...
WANDB_ARGS=(
   ...
   --tb-experiment-name qwen3-4b-GRPO-gpu8-hybrid-async-${now}
   # --use-wandb
...
ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
   ...
   --hybrid \
    ...
    "${EVAL_ARGS[@]}" \
    "${SGLANG_ARGS[@]}" \
    "${MISC_ARGS[@]}"  2>&1 | tee log/qwen3-4b-GRPO-gpu8-hybrid-async-${now}.log
