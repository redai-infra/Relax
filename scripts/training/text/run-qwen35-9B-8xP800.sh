...
export MODEL_DIR="${MODEL_DIR:-/workdir/model/qwen3.5-9B}" # 存放模型权重、训练样本的共同父目录
...
if [ -z "${RELAX_ENTRYPOINT_MODE:-}" ]; then
    source "/workspace/Relax/scripts/entrypoint/local.sh"
fi
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3.5-p800-9B-yqh
