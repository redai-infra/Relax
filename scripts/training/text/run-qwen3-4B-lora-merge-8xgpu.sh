...
PROJECT_NAME="${PROJECT_NAME:=Relax/dev/qwen3_4b_lora_merge}"
...
WANDB_ARGS=(
   ...
   --tb-experiment-name qwen3-4b-lora-${now}
   # --use-wandb
...
ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
   ...
   --colocate \
    ...
    "${SGLANG_ARGS[@]}" \
    "${LORA_ARGS[@]}" \
    "${MISC_ARGS[@]}"  2>&1 | tee log/qwen3-4b-lora-merge-GRPO-gpu8-${now}.log
