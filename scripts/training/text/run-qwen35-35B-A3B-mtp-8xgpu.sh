...
EXP_NAME="${EXP_NAME:-Qwen3.5-35B-A3B-test-mtp-${now}}"
...
CKPT_ARGS=(
   ...
   --load ${EXP_DIR}/Qwen3.5-35B-A3B_mtp_mcore_8xgpu/
   --save ${EXP_DIR}/Qwen3.5-35B-A3B_mtp_mcore_8xgpu/
   --save-interval 100
...
SGLANG_ARGS=(
   ...
   # SGLang >=0.5.9 routes Qwen3.5-MoE's hybrid attention through MambaRadixCache.
   # Default `no_buffer` strategy is incompatible with speculative decoding (EAGLE/MTP),
   # so we must opt into `extra_buffer` + spec-v2 overlap scheduler. See SGLang
   # `_handle_mamba_radix_cache` (server_args.py).
...
ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://${HOST_IP}:8265" \
   ...
   "${EVAL_ARGS[@]}" \
   "${SGLANG_ARGS[@]}" \
   "${MISC_ARGS[@]}"  2>&1 | tee log/qwen35-35B-A3B-mtp-GRPO-gpu8-sync-${now}.log
