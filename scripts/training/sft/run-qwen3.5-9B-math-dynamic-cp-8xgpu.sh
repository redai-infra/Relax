...
#
# Usage:
#   bash scripts/training/sft/run-qwen3.5-9B-math-dynamic-cp-8xgpu.sh          # dynamic CP
#   CP=4 bash scripts/training/sft/run-qwen3.5-9B-math-dynamic-cp-8xgpu.sh     # static CP=4
#   CP=1 bash scripts/training/sft/run-qwen3.5-9B-math-dynamic-cp-8xgpu.sh     # no CP
...
if [ -n "${CP}" ]; then
    EXP_NAME=qwen3.5-9b-sft-math-cp${CP}-gpu8
else
    EXP_NAME=qwen3.5-9b-sft-math-dyncp-gpu8
fi
...
SAVE_DIR="${SAVE_DIR:=${SCRIPT_DIR}/../../../checkpoints/qwen3.5-9B-math-sft-dyncp}"
