...
#
# Qwen3-VL-30B-A3B 8xGPU colocate training script.
#
# Usage:
#   bash scripts/training/multimodal/run-qwen3-vl-30B-A3B-8xgpu.sh
...
CKPT_ARGS=(
   --hf-checkpoint ${MODEL_DIR}/Qwen3-VL-30B-A3B-Instruct
   --ref-load ${MODEL_DIR}/Qwen3-VL-30B-A3B-Instruct
...
ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
   ...
   "${PERF_ARGS[@]}" \
   "${SGLANG_ARGS[@]}" \
   "${MISC_ARGS[@]}"  2>&1 | tee log/qwen3-vl-30B-A3B-GRPO-gpu8-${now}.log
