...
MaaS 只用于评测或生成 teacher trajectory，不能直接替换 Relax 的 on-policy rollout model 参与
GRPO 更新。Relax 训练需要当前 actor 的权重和 logprob；远端 MaaS 模型与本地 actor 不是同一个策略。

先把 Gym 切换到 train mode，并将 MaaS hostname 加入精确 callback allowlist。`--proxy` 只用于
Gym/R2E/OpenHands 的依赖下载，`--callback-proxy` 专门用于 Gateway 到 MaaS 的模型请求：

```bash
export MAAS_HOST="<maas-host>"
export CALLBACK_PROXY="http://<proxy-host>:<proxy-port>"

bash examples/nemo_gym_agentic/recipes/r2e-gym/start_r2e_gym_remote.sh \
  --gym-host "${GYM_HOST}" \
  --data-dir "${R2E_DATA_DIR}" \
  --mode train \
  --callback-host "${MAAS_HOST}" \
  --image "${NEMO_GYM_IMAGE}" \
  --repo-dir "${RELAX_REPO_ON_DOCKER_HOST}" \
  --proxy "${https_proxy}" \
...
如果 Gym 主机能直接访问 MaaS，省略 `--callback-proxy`。显式 callback proxy 启用后，Gateway 只接受
HTTPS model endpoint；代理 URL 不允许包含账号密码、path、query 或 fragment。单次模型请求默认最多
等待 600 秒，可通过 `NEMO_GYM_CALLBACK_TIMEOUT_S` 调整。

另开 shell，从环境变量注入凭据。脚本不会把 API key 写入结果文件：

```bash
export MAAS_BASE_URL="https://${MAAS_HOST}/v1"
export MAAS_MODEL="<tool-capable-model>"
export MAAS_API_KEY="<从 secret manager 读取>"
export MAAS_USER_EMAIL="<maas-user-email>"
export MAAS_APP_ID="<maas-app-id>"

python examples/nemo_gym_agentic/recipes/r2e-gym/eval_r2e_gym_maas.py \
  --gateway-url "http://${GYM_HOST}:28100" \
  --task-jsonl "${R2E_DATA_DIR}/r2e_gym_train.jsonl" \
  --limit 1 \
  --max-concurrency 1 \
  --max-tokens 8192 \
  --output-jsonl "${R2E_DATA_DIR}/r2e_gym_maas_results.jsonl"
```

脚本复用 Gateway 的 retry、heartbeat 和 abort 生命周期；收到 SIGINT/SIGTERM 时只 abort 本脚本创建的
trial。结果逐条写入并 flush。MaaS direct-eval 会额外启用 artifact capture：

- 完整 Responses `output`，包括 tool call、tool output 和 assistant message；
- `resolved`、`patch_exists`、`model_patch` 和 timeout/OOM 分类；
- OpenHands `test_result` 与 `skip_reason`；
- `trajectories/`、`apptainer_logs/` 和实际生成的 evaluator report。

artifact 默认持久化到 `${R2E_DATA_DIR}/artifacts/<request-hash>/`，文件权限为 `0600`，目录权限为
`0700`。Gateway 只回传 `artifact_ref`；评测脚本读取 manifest 后将其合并到输出 JSONL 的
`.artifact` 字段。训练 trial 默认不 capture，避免批量训练产生额外 I/O。
...
jq -c '{
  ...
  openhands_result: .artifact.openhands_result
}' \
  "${R2E_DATA_DIR}/r2e_gym_maas_results.jsonl"
```

查看实际工具调用：

```bash
jq -c '.artifact.response_output[]
...
1. trial 为 `completed`，说明 Gateway 已将该次 agent run 收敛到 terminal 状态；
2. metrics 中出现模型调用和 tool call，说明 MaaS 返回了兼容的结构化工具调用；
3. `patch_exists=true`，说明 OpenHands 已成功收集模型修改；
4. reward=1 且 `resolved=true`，说明 MaaS 生成的 patch 已进入 evaluator 并通过该题测试。

普通文本 `/chat/completions` 请求成功不能证明模型支持 R2E-Gym；R2E-Gym 必须依赖结构化 tool call。
