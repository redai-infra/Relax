...
if envs.SGLANG_RESIZE_RESAMPLE.is_set() and RESIZE_RESAMPLE is None:
    logger.warning(
        f"Invalid RESIZE_RESAMPLE value: '{envs.SGLANG_RESIZE_RESAMPLE.get()}'. Ignoring and using default."
    )
...
class QwenVLImageProcessor(SGLangBaseProcessor):
    ...
    async def process_mm_data_async(
        ...
        **kwargs,
        ...
        # which can alter the token count. We restore
        # the original input_ids afterwards so the engine's token bookkeeping stays
